...
def eval_ind(params):
    print params
    return self.evaluate_individual_gp(params[0], params[1], params[2], params[3], params[4])

def evaluate_population_gp(self, population_gp, cache=True, pop_size=5, generations=5):
    if self.parallel:
        # For running local:
        cluster = dispy.JobCluster(self.eval_ind, depends=[self], ip_addr='127.0.0.1')
        # For running on LISA:
    ...
    for i in range(0, len(population_gp)):
        ...
        if self.parallel:
            ...
            job = cluster.submit(random.random())
                                 #[m, i, cache, pop_size, generations])
            job.id = i
            jobs.append(job)
        else:
            temp = self.evaluate_individual_gp(m, i, cache, pop_size, generations)
